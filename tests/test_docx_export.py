...
def test_uebungsblatt_ist_gueltige_docx(tmp_path, liste):
    pfad = docx_export.uebungsblatt_schreiben(
        tmp_path / "ub.docx", "TK", "Doppelkonsonanten", ["07"], liste,
...
def test_uebungsblatt_trennt_vorder_und_rueckseite(tmp_path, liste):
    """Vorderseite Übung, Rückseite Test – ohne Seitenumbruch stimmt der
    beidseitige Druck nicht."""
    pfad = docx_export.uebungsblatt_schreiben(
        tmp_path / "ub.docx", "TK", "Titel", ["07"], liste,
...
def test_uebungsblatt_enthaelt_name_datum_und_kategorie(tmp_path, liste):
    pfad = docx_export.uebungsblatt_schreiben(
        tmp_path / "ub.docx", "TK", "Kürzemarkierung", ["07"], liste,
...
def test_loesungen_standardmaessig_nicht_im_dokument(tmp_path, liste):
...
def test_loesungsblatt_nur_auf_ausdruecklichen_wunsch(tmp_path, liste):
    pfad = docx_export.uebungsblatt_schreiben(
        tmp_path / "ub.docx", "TK", "Titel", ["07"], liste,
...
def test_leeres_loesungsfeld_erzeugt_keine_dritte_seite(tmp_path, liste):
    pfad = docx_export.uebungsblatt_schreiben(
        tmp_path / "ub.docx", "TK", "Titel", ["07"], liste,
...
def test_mehrzeiliger_inhalt_bleibt_erhalten(tmp_path, liste):
    inhalt = "Zeile eins\nZeile zwei\n\nZeile vier"
    pfad = docx_export.uebungsblatt_schreiben(
        tmp_path / "ub.docx", "TK", "Titel", ["07"], liste, inhalt, "Test",
    )
    text = _volltext(Document(str(pfad)))
...
def test_verzeichnis_wird_bei_bedarf_angelegt(tmp_path, liste):
    pfad = docx_export.uebungsblatt_schreiben(
        tmp_path / "neu" / "tiefer" / "ub.docx", "TK", "Titel", ["07"], liste,
        "Übung", "Test",
    )
    assert pfad.exists()
...
def test_informationsblatt_listet_fehler_in_tabelle(tmp_path, liste):
    ...
    pfad = docx_export.informationsblatt_schreiben(
        tmp_path / "info.docx", "TK", "Wald-Diktat", "2026-03-05", fehler, liste,
...
def test_informationsblatt_zaehlt_kategorien(tmp_path, liste):
    fehler = [{"wort_original": "a", "wort_schueler": "b", "kategorie_nr": "07",
               "kontext": ""} for _ in range(3)]
    pfad = docx_export.informationsblatt_schreiben(
        tmp_path / "info.docx", "TK", "D", "2026-03-05", fehler, liste,
    )
    assert "3×" in _volltext(Document(str(pfad)))


def test_informationsblatt_ohne_fehler(tmp_path, liste):
    pfad = docx_export.informationsblatt_schreiben(
        tmp_path / "info.docx", "TK", "D", "2026-03-05", [], liste,
    )
    assert "Keine Fehler erfasst." in _volltext(Document(str(pfad)))


def test_diktattext_kommt_auf_eigene_seite(tmp_path, liste):
    pfad = docx_export.informationsblatt_schreiben(
        tmp_path / "info.docx", "TK", "D", "2026-03-05", [], liste,
...
def test_verlaufsbericht_mit_trendzeilen(tmp_path, liste):
    ...
    pfad = docx_export.verlaufsbericht_schreiben(
        tmp_path / "bericht.docx", "TK", "Jan–Mär 2026", zeilen, liste,
...
def test_verlaufsbericht_ohne_daten(tmp_path, liste):
    pfad = docx_export.verlaufsbericht_schreiben(
        tmp_path / "bericht.docx", "TK", "–", [], liste,
    )
    assert "Noch keine auswertbaren Daten" in _volltext(Document(str(pfad)))


def test_verlaufsbericht_bettet_diagramm_ein(tmp_path, liste):
    from rstrainer import charts
    from conftest import diktatpunkte

    figur, _ = charts.verlauf_linien(diktatpunkte(3), {"07": [1, 2, 3]}, liste)
    bild = charts.speichern(figur, tmp_path / "diagramm.png")
    pfad = docx_export.verlaufsbericht_schreiben(
        tmp_path / "bericht.docx", "TK", "Zeitraum", [], liste, diagramm_pfad=bild,
    )
...
def test_fehlendes_diagramm_bricht_nicht_ab(tmp_path, liste):
    pfad = docx_export.verlaufsbericht_schreiben(
        tmp_path / "bericht.docx", "TK", "Zeitraum", [], liste,
